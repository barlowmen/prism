...
import { renderDocxToHtml } from "@/lib/docx-preview";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";

/**
 * GET /api/jobs/[id]/docx                 → most-recent DOCX rendered to HTML
 * GET /api/jobs/[id]/docx?name=Foo.docx   → a specific DOCX
 */
export async function GET(
  ...
  const rendered = await renderDocxToHtml(abs);
  if (!rendered) {
    return NextResponse.json({ error: "docx_disappeared" }, { status: 404 });
  }
